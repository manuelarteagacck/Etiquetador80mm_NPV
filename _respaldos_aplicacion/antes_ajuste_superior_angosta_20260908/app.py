...
def _estimate_label_height_dots(
    ...
    if section == "angosta" and auto_individual_print:
        desc_advance = _param_int(params, "descripcion_advance_auto", 40, 0)
    else:
        desc_advance = _param_int(
            params,
            "descripcion_advance",
            44 if section == "normal" else 38,
            0,
        )
    desc_rect_height = _param_int(
        params,
        "descripcion_rect_height",
        46 if section == "normal" else 40,
        1,
    )
    ...
    try:
        ...
        for i in range(copies):
            ...
            try:
                ...
                desc_advance = _param_int(small_params, "descripcion_advance_auto", 40, 0) if auto_individual_print else _param_int(small_params, "descripcion_advance", 38, 0)
                desc_rect_height = _param_int(small_params, "descripcion_rect_height", 40, 1)
                desc = (item_dict.get("DESCRIPCION") or "").upper()
                _, desc_flags = _description_font_gdi(
                    hDC,
                    desc,
...
class LabelPrintPreviewWindow(tk.Toplevel):
    def __init__(self, parent, item: Dict[str, Any], individual: bool = False):
        ...
        if self.item.get("PRECIO_ESPECIAL"):
            ...
            label_height = 240 if individual else 280
    ...
    def _draw_label(self, label_width: int, label_height: int):
        ...
        if special_price:
            ...
            self.canvas.create_text(
                center_x,
                y1 + (40 if self.individual else 50),
                text=price,
                width=text_right - text_left,
                anchor="n",
                justify="center",
            ...
            footer_y = y1 + (100 if self.individual else 132)
